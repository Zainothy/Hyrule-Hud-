...
async function checkAndMigrateVersion() {
  ...
  if (stored.schemaVersion > SCHEMA_VERSION) {
    ...
    const { response } = await require("electron").dialog.showMessageBox({
      ...
      detail:
        "This version is older than your existing install. Settings cannot be carried forward.\n\nTo recover, manually download the latest version.",
  ...
  if (stored.schemaVersion < SCHEMA_VERSION) {
    ...
    const { response } = await require("electron").dialog.showMessageBox({
      ...
      detail:
        "This update changes the configuration format. Your settings will be reset.",
      buttons: ["Reset & Continue", "Quit"],
      defaultId: 0,
      cancelId: 1,
    });
  ...
  if (stored.appVersion !== app.getVersion()) {
    for (const f of OBSOLETE_APPDATA_FILES) {
      try {
        fs.unlinkSync(path.join(APP_DATA_DIR, f));
      } catch {
        /* already gone */
      }
    }
...
function wipeAppData() {
  try {
    ...
    for (const entry of entries) {
      fs.rmSync(path.join(APP_DATA_DIR, entry), {
        recursive: true,
        force: true,
      });
    }
...
function initAutoUpdater() {
  ...
  autoUpdater.on("update-available", async (info) => {
    ...
    if (response === 0) {
      autoUpdater
        .downloadUpdate()
        .catch((e) => logError("[updater] downloadUpdate error: " + e.message));
    }
  });
  ...
  autoUpdater
    .checkForUpdates()
    .catch((e) => logError("[updater] checkForUpdates error: " + e.message));
}
    ...
    items.push({
      ...
      click: () => {
        ...
        autoUpdater
          .downloadUpdate()
          .catch((e) =>
            logError("[updater] downloadUpdate error: " + e.message),
          );
      },
    });
...
async function reconfigure() {
  ...
  _reconfiguring = true;
  try {
    const result = await openSetupWindow(
      currentConfig,
      false,
      scanCemuSavePaths,
    );
...
app.whenReady().then(async () => {
  ...
  if (isFirstRun) {
    ...
    const suggested =
      candidates.length > 0 ? { savePath: candidates[0], port: 8080 } : null;
