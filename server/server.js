...
// Eight save slots total, matching BotW's own on-disk convention:
//   0–5 = Normal Mode (0 manual, 1–5 auto-saves)
//   6–7 = Master Mode (0 manual, 1 auto-save)
...
// Returns [{ index, mode, filePath, mtimeMs }] for every slot that currently
// exists on disk, sorted most-recent first. Lets the UI show every save the
// player has (Normal + Master Mode side by side) instead of only ever
// guessing at one.
function listSaveSlots(callback) {
    const slots = getSaveSlots();
    let remaining = slots.length;
    const found = [];
    slots.forEach((filePath, index) => {
        fs.stat(filePath, (err, stats) => {
            if (!err && stats.isFile()) {
                found.push({ index, mode: slotMode(index), filePath, mtimeMs: stats.mtimeMs });
            }
            if (--remaining === 0) {
...
// Find the save slot to serve: the user's manually pinned slot if set and
// still present, otherwise whichever readable slot was modified most
// recently (across ALL 8 slots, Normal and Master Mode alike).
// Calls callback(filePath, mtimeMs, slotIndex) or callback(null, null, null).
function getMostRecentSave(callback) {
    listSaveSlots((slots) => {
        if (slots.length === 0) return callback(null, null, null);
        const pinned = readState().pinnedSlot;
        const chosen =
            pinned != null ? slots.find((s) => s.index === pinned) : null;
        const winner = chosen || slots[0];
        callback(winner.filePath, winner.mtimeMs, winner.index);
    });
}
...
app.get('/api/mtime', (req, res) => {
    getMostRecentSave((filePath, mtime, slotIndex) => {
        res.setHeader('Cache-Control', 'no-store');
        const stateVersion = readState().stateVersion || 0;

        if (!filePath) {
            _saveValidationCache = { mtime: null, status: 'not_found' };
            res.json({ mtime: null, stateVersion, saveStatus: 'not_found', slot: null, mode: null });
            return;
        }

        const slotMeta = { slot: slotIndex, mode: slotMode(slotIndex) };

        // Cache hit — mtime unchanged, reuse previous validation result
        if (mtime === _saveValidationCache.mtime) {
            res.json({ mtime, stateVersion, saveStatus: _saveValidationCache.status, ...slotMeta });
            return;
        }
        ...
        fs.readFile(filePath, (err, buf) => {
            ...
            res.json({ mtime, stateVersion, saveStatus: status, ...slotMeta });
        });
    });
});

// GET /api/slots — lists every save slot present on disk (Normal + Master Mode),
// so the UI can offer a manual picker instead of relying on auto-detection alone.
app.get('/api/slots', (req, res) => {
    listSaveSlots((slots) => {
        const pinnedSlot = readState().pinnedSlot ?? null;
        res.setHeader('Cache-Control', 'no-store');
        res.json({
            ok: true,
            pinnedSlot,
...
// PATCH /api/state/pinned-slot — manually lock the viewer onto a specific slot
// (0–7, or null to go back to auto "most recently modified" detection).
app.patch('/api/state/pinned-slot', (req, res) => {
    const { slot } = req.body || {};
    if (slot !== null && (!Number.isInteger(slot) || slot < 0 || slot >= TOTAL_SLOTS)) {
...
app.patch('/api/state/hidden-types', (req, res) => {
    ...
    const patch = { hiddenTypes: Array.from(set) };
    if (hidden) patch.forcedVisibleTypes = Array.from(forcedSet);
    res.json({
        ok: true,
        state: writeStateAndBroadcast(patch)
    });
});
...
app.patch('/api/state/forced-visible-types', (req, res) => {
    ...
    const patch = { forcedVisibleTypes: Array.from(set) };
    if (forceVisible) patch.hiddenTypes = Array.from(hiddenSet);
    res.json({
        ok: true,
        state: writeStateAndBroadcast(patch)
    });
