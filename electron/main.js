...
const APP_DATA_DIR = path.join(os.homedir(), 'AppData', 'Roaming', 'hyrule-hud');
...
const REPO_URL = 'https://github.com/YOUR_GITHUB_USERNAME/hyrule-hud';
...
function buildMenu(serverOk, updateVersion = null) {
    ...
    if (updateVersion) {
        items.push({ label: `Install Update v${updateVersion}…`, click: () => {
            tray.displayBalloon({ title: 'Hyrule HUD', content: 'Downloading update…', iconType: 'info' });
            autoUpdater.downloadUpdate().catch((e) => logError('[updater] downloadUpdate error: ' + e.message));
        }});
